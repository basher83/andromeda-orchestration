...
# Verify Consul health checks
consul catalog services -tags | grep certificate-monitor

# Check Prometheus metrics
curl localhost:9793/metrics | grep x509

# View certificate inventory
cat /var/lib/cert-inventory.json | jq .

# Check latest daily report
ls -la /var/log/cert-report-*.txt
tail -20 /var/log/cert-report-*.txt
```
